...
if(isset($_POST['add_product'])){
    ...
    $product_image_folder = 'Fertilizers/'.$product_image;

    if(empty($product_name) || empty($product_price) || empty($product_image)){
        $message[] = 'please fill out all';
    }else{
        $insert = "INSERT INTO fertilizer ( price,pro_name, image) VALUES( '$product_price','$product_name', '$product_image')";
...
if(isset($_GET['delete'])){
    $id = $_GET['delete'];
    mysqli_query($conn, "DELETE FROM fertilizer WHERE pro_id = $id");
    header('location:adminfertilizers.php');
};
...
<div class="container">
    ...
    <div class="admin-product-form-container">
        ...
        <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
            ...
            <input type="number" placeholder="enter fertilizer price" name="product_price" class="box">
            <input type="file" accept="image/png, image/jpeg, image/jpg" name="product_image" class="box">
            <input type="submit" class="btn" name="add_product" value="add fertilizer">
        </form>
    ...
    <div class="product-display">
        <table class="product-display-table">
            ...
            <?php while($row = mysqli_fetch_assoc($select)){ ?>
                <tr>
                    <td><img src="Fertilizers/<?php echo $row['image']; ?>" height="100" alt=""></td>
                    <td><?php echo $row['pro_name']; ?></td>
                    <td>$<?php echo $row['price']; ?></td>
                    <td>
                        <a href="fertilizerUbdate.php?edit=<?php echo $row['pro_id']; ?>" class="btn"> <i class="fas fa-edit"></i> edit </a>
                        <a href="adminfertilizers.php?delete=<?php echo $row['pro_id']; ?>" class="btn"> <i class="fas fa-trash"></i> delete </a>
                    </td>
                </tr>
            <?php } ?>
        </table>
